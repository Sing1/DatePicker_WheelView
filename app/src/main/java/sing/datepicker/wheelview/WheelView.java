...
//     11. void setOnSelectListener(OnSelectListener onSelectListener) 对WheelView设置监听，在 滑动过程 或者 滑动停止 返回数据信息。
    ...
    // 线的默认颜色
    private int lineColor = 0xE2E2E2;
    // 线的默认宽度
    private float lineWidth = 1f;
    // 默认字体
    private float normalFont = 32.0f;
    // 选中的时候字体
    private float selectedFont = 36.0f;
    // 单元格高度
    private int unitHeight = 90;
    // 显示多少个内容
    private int itemNumber = 5;
    // 默认字体颜色
    private int normalColor = 0xFF999999;
    // 选中时候的字体颜色
    private int selectedColor = 0xFF41B0FB;
    // 蒙板高度
    private float maskHight = 35.0f;
    // 选择监听
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        unitHeight = (int) attribute.getDimension(R.styleable.WheelView_unitHight, 90);
        normalFont = attribute.getDimension(R.styleable.WheelView_normalTextSize, 32.0f);
        selectedFont = attribute.getDimension(R.styleable.WheelView_selectedTextSize, 36.0f);
        itemNumber = attribute.getInt(R.styleable.WheelView_itemNumber, 5);
        normalColor = attribute.getColor(R.styleable.WheelView_normalTextColor, 0xff999999);
        selectedColor = attribute.getColor(R.styleable.WheelView_selectedTextColor, 0xFF41B0FB);
        lineColor = attribute.getColor(R.styleable.WheelView_lineColor, 0xFFE2E2E2);
        lineWidth = attribute.getDimension(R.styleable.WheelView_lineHeight, 1f);
        maskHight = attribute.getDimension(R.styleable.WheelView_maskHight, 35.0f);
        ...
        if (move > 0) {
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).isSelected()) {
                    ...
                    if (onSelectListener != null)
                        onSelectListener.endSelect(this, itemList.get(i).id, itemList.get(i).itemText);
                    break;
                }
            }
        } else {
    ...
    public interface OnSelectListener {
        ...
        // 选中的内容
        void selecting(View view, int id, String text);
    }
}